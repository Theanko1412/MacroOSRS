...
class MacroRecorderApp:
    def __init__(self, root):
        ...
        self.tree = ttk.Treeview(root, columns=(
            "name", "action", "delay_text", "dist", "min", "max", "click_dist", "area"), show="headings")
        ...
        self.context_menu.add_command(
            label="Delete Command", command=self.delete_command)
        self.context_menu.add_command(
            label="Modify Command", command=self.modify_command)
        self.context_menu.add_command(
            label="Edit Condition", command=self.edit_condition)
        self.context_menu.add_command(
            label="Edit Name", command=self.edit_action_name)
        self.context_menu.add_command(
            label="Modify Time Distribution", command=self.edit_time_distribution)
        self.context_menu.add_command(
            label="Modify Click Area", command=self.edit_click_area)
        ...
        self.record_button = tk.Button(
            control_frame, text="Record Macro", command=self.start_recording)
        ...
        self.play_button = tk.Button(
            control_frame, text="Play Macro", command=self.play_macro)
        self.play_button.grid(row=0, column=2)
        tk.Button(control_frame, text="Save Macro",
                  command=self.save_macro).grid(row=0, column=3)
        tk.Button(control_frame, text="Load Macro",
                  command=self.load_macro).grid(row=0, column=4)
        self.add_button = tk.Button(
            control_frame, text="Add Command", command=self.add_command)
        self.add_button.grid(row=0, column=5)
        tk.Button(control_frame, text="Add Condition",
                  command=self.add_condition).grid(row=0, column=6)
        tk.Button(control_frame, text="Move Mouse",
                  command=self.add_move_command).grid(row=0, column=7)
        tk.Button(control_frame, text="Reset",
                  command=self.reset_macro).grid(row=0, column=8)
        reorder_frame = tk.Frame(root)
        reorder_frame.pack()
        tk.Button(reorder_frame, text="Move Up",
                  command=self.move_up).pack(side=tk.LEFT)
        tk.Button(reorder_frame, text="Move Down",
                  command=self.move_down).pack(side=tk.LEFT)
        tk.Button(reorder_frame, text="Insert Timer",
                  command=self.insert_timer).pack(side=tk.LEFT)

        loop_frame = tk.Frame(root)
        loop_frame.pack(pady=5)
        tk.Label(loop_frame, text="Loop count:").grid(row=0, column=0)
        tk.Entry(loop_frame, textvariable=self.loop_count,
                 width=5).grid(row=0, column=1)

        status_frame = tk.Frame(root)
        status_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=2)

        self.loop_status = tk.Label(
            status_frame, text="", width=20, anchor="w")
        ...
        self.time_status = tk.Label(
            status_frame, text="", width=20, anchor="e")
    ...
    def add_move_command(self):
        ...
        self.status_label.config(
            text="Click anywhere to set mouse position (ESC to cancel)")
    ...
    def prepare_for_pixel_selection(self):
        ...
        # Mouse listeners
        self.area_mouse_listener = mouse.Listener(
            on_click=self.on_pixel_selection_click)
        self.area_mouse_listener.start()

        # Keyboard listener
        self.area_keyboard_listener = keyboard.Listener(
            on_press=self.on_area_selection_key)
    ...
    def edit_condition(self):
        ...
        tk.Entry(edit_dialog, textvariable=name_var).grid(
            row=0, column=1, columnspan=2, sticky="ew")

        # Timeout setting
        tk.Label(edit_dialog, text="Timeout (0 for infinite):").grid(
            row=1, column=0)
        timeout_var = tk.DoubleVar(value=condition["timeout"])
        tk.Entry(edit_dialog, textvariable=timeout_var).grid(row=1, column=1)

        # Check interval
        tk.Label(edit_dialog, text="Check interval (seconds):").grid(
            row=2, column=0)
        ...
        tk.Radiobutton(edit_dialog, text="Yes", variable=match_var,
                       value=True).grid(row=3, column=1)
        tk.Radiobutton(edit_dialog, text="No", variable=match_var,
                       value=False).grid(row=3, column=2)

        # Position display
        x, y = condition["pos"]
        tk.Label(edit_dialog, text=f"Position: ({x}, {y})").grid(
        ...
        tk.Button(edit_dialog, text="Save", command=save_changes).grid(
    ...
    def add_command(self):
        self.add_button.config(state=tk.DISABLED)
        self.status_label.config(
            text="Press any key or click anywhere to add command (ESC to cancel)")
    ...
    def modify_command(self):
        ...
        self.status_label.config(
            text="Press new key/click to modify command (ESC to cancel)")
    ...
    def capture_command(self, modify_index, is_move_command=False):
        ...
        self.capture_keyboard_listener = keyboard.Listener(
    ...
    def handle_captured_command(self):
        ...
        if self.captured_event:
            ...
            self.refresh_tree()
            self.status_label.config(
                text="Command added/modified successfully")
        else:
    ...
    def edit_click_area(self):
        ...
        # Update click distribution type immediately
        self.macro[index]["click_dist"] = dist
        ...
        if dist == "fixed":
            ...
            self.area_selection_start = None
            self.status_label.config(
                text="Please click the TOP-LEFT corner of the click area...")
    ...
    def prepare_for_area_selection(self):
        ...
        self.area_mouse_listener = mouse.Listener(
            on_click=self.on_area_selection_click)
        self.area_mouse_listener.start()
    ...
    def on_area_selection_click(self, x, y, button, pressed):
        ...
        if not self.area_selection_start:
            ...
            self.status_label.config(
                text="Now click the BOTTOM-RIGHT corner of the click area...")
            ...
            # Get the already updated click_dist
            click_dist = self.macro[index]["click_dist"]
            ...
            self.area_selection_mode = False
            self.status_label.config(
                text=f"Area selection completed: {top_left} to {bottom_right}")
            self.refresh_tree()
            self.root.deiconify()  # Show main window again
    ...
    def start_recording(self):
        ...
        self.click_counter = {"left": 0, "right": 0,
                              "middle": 0}  # Reset click counters
    ...
    def edit_action(self, event):
        ...
        if dist == "fixed":
            ...
            if val is not None:  # Check if user didn't cancel
                self.macro[index].update(
                    {"dist": dist, "delay": val, "min": val, "max": val})
        else:
            ...
            if min_val is not None:  # Check if user didn't cancel
                ...
                if max_val is not None:  # Check if user didn't cancel
                    self.macro[index].update(
                        {"dist": dist, "min": min_val, "max": max_val})
        self.refresh_tree()
    ...
    def get_click_position(self, event):
        ...
        if click_dist == "uniform":
            ...
            distance = random.expovariate(
                4 / (width + height))  # Exponential distribution
    ...
    def perform_human_click(self, mc, target_x, target_y, button):
        ...
        # Random delay before main movement
    ...
    def play_macro(self):
        def play():
            ...
            self.status_label.config(
                text="Playing macro... (Press CTRL+ESC to stop)")
            ...
            try:
                for loop_num in range(1, self.total_loops + 1):
                    ...
                    for event_idx, event in enumerate(self.macro):
                        ...
                        if event["type"] in ["click", "key", "delay", "move"]:
                            event_name = event.get(
                                "name", f"Action {event_idx+1}")
                            delay = self.get_delay(event)

                            # Update status with delay information
                            self.status_label.config(
                                text=f"Playing: {event_name} (waiting {delay:.2f}s)")
                            time.sleep(delay)
                            ...
                            if event["type"] == "move":
                                ...
                                button_str = event["button"].split(
                                    ".")[-1].split(":")[0].strip(">").lower()
                                button = getattr(
                                    mouse.Button, button_str, mouse.Button.left)
                                mc = mouse.Controller()
                                self.perform_human_click(mc, x, y, button)
                                self.status_label.config(
                                    text=f"Executed: {event_name}")
                            elif event["type"] == "key":
                                key_str = event["key"]
                            ...
                            self.status_label.config(
                                ...
                                (f"(Timeout in {event['timeout']}s)" if event["timeout"]
                                 > 0 else "(Infinite wait)")
                            )
                            ...
                            while not condition_met and not self.abort_playback:
                                ...
                                current_red = (color >> 16) & 0xff
                                current_green = (color >> 8) & 0xff
                                current_blue = color & 0xff
                                current_hex = '#%02x%02x%02x' % (current_red, current_green, current_blue)

                                # Check condition
                                if event["match"]:
                                    condition_met = (
                                        current_hex.lower() == event["color"].lower())
                                else:
                                    condition_met = (
                                        current_hex.lower() != event["color"].lower())
                    ...
                    if loop_num < self.total_loops and not self.abort_playback:
                        self.status_label.config(
                            text=f"Loop {loop_num} complete. Starting next loop...")
            ...
            if not self.abort_playback:
                elapsed = time.time() - self.playback_start_time
                self.status_label.config(
                    text=f"Macro playback completed in {elapsed:.2f} seconds")
            else:
    ...
    def insert_timer(self):
        dist = simpledialog.askstring(
            "Distribution", "Enter delay distribution (fixed/uniform/poisson):", initialvalue="fixed")
        if not dist:
            return

        # Get timer name
        timer_name = simpledialog.askstring(
            "Timer Name", "Enter timer name:", initialvalue=f"Timer {len(self.macro)+1}")
        if timer_name is None:  # User cancelled
            return

        if dist == "fixed":
            val = simpledialog.askfloat(
                "Delay", "Enter delay (sec):", initialvalue=0.5)
            ...
            min_val = simpledialog.askfloat(
                "Min", "Enter minimum delay (sec):", initialvalue=0.1)
            if min_val is None:  # User cancelled
                return
            max_val = simpledialog.askfloat(
                "Max", "Enter maximum delay (sec):", initialvalue=1.0)
